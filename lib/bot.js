...
class Bot {
  ...
  sendToDiscord(author, channel, text) {
    ...
    const withMentions = withFormat.replace(/@([^\s#]+)#(\d+)/g, (match, username, discriminator) => {
      ...
      // If there are no spaces in the nickname then this is redundant
      const checkMatch = function (matchString, matchValue) {
        const words = matchString.split(" ");
        const wordPrefixes  = words.reduce((arr, cur) => {
          if(arr.length === 0) return [cur];
          arr.push(arr[arr.length-1] + " " + cur);
          return arr;
        }, []);
        wordPrefixes.forEach((prefixString) => {
          if (prefixString.length > matchLength && Bot.caseComp(reference, prefixString)) {
            matchLength = prefixString.length;
            bestMatch = matchValue;
          }
        });
      };

      // check users by nickname
      guild.members.forEach((member) => {
        // Do not check against usernames any more
